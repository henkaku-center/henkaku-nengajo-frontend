...
import { useRegisterOmamori } from '@/hooks/useOmamoriContract'
...
interface OmamoriTokenMetadata {
  name: string
  image: string
...
const CreateOmamoriForm: FC = () => {
  ...
  const {
    ...
    writeAsync,
    registeredTokenId
  } = useRegisterOmamori(Number(watch('maxSupply')), metadataURI)
  ...
  useEffect(() => {
    const callback = async () => {
      if (isSuccess && registeredTokenId) {
        ...
        router.push(`/omamori/${registeredTokenId}`)
      }
    }
    callback()
  ...
  const submit = async (data: FormData) => {
    try {
      ...
      const metadataJson: OmamoriTokenMetadata = {
        name: data.name,
    ...
    <Box>
      <form onSubmit={handleSubmit(submit)}>
        <FormControl mt={5} isRequired>
          <FormLabel mt="1em" htmlFor="omamoriName">
            {o('CREATE.NEW_OMAMORI_TITLE_LABEL')}
          </FormLabel>
          <Controller
            control={control}
            name="name"
            ...
            render={({ field: { onChange, value }, fieldState }) => (
              <>
                <Input
                  ...
                  type="text"
                  isRequired={true}
                  placeholder={o('CREATE.NEW_OMAMORI_TITLE_LABEL')}
                  onChange={onChange}
                />
        ...
        <FormControl mt={5}>
          <FormLabel mt="1em" htmlFor="secretMessage">
            {o('CREATE.NEW_OMAMORI_SECRET_MESSAGE_LABEL')}
          </FormLabel>
          <Controller
            control={control}
            name="secretMessage"
...
export default CreateOmamoriForm
